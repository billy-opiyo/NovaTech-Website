...
export default function AccountPage() {
	...
	return <div className="space-y-8"><h1 className="text-3xl font-bold">My Account</h1><div className="grid gap-8 lg:grid-cols-3"><aside><div className="glass-card p-6 text-center"><AccountAvatar name={user?.name} email={user?.email} image={user?.image} className="mx-auto mb-4 h-20 w-20 text-2xl"/><h2 className="text-xl font-semibold">{user?.name || "Customer"}</h2><p className="text-sm text-gray-500">{user?.email || ""}</p><button type="button" disabled={signingOut} aria-busy={signingOut} onClick={() => void handleSignOut()} className="mt-4 inline-flex items-center gap-1 text-sm text-red-500 disabled:opacity-60">{signingOut ? <LoaderCircle size={14} className="animate-spin" aria-hidden="true" /> : <LogOut size={14} aria-hidden="true"/>}{signingOut ? "Signing out…" : "Sign Out"}</button></div><div className="mt-6 space-y-2">{links.map(({ icon: Icon, label, href }) => <Link key={href} href={getStoreRouteHref(store, href)} className="glass-card flex items-center justify-between p-4 transition hover:bg-white/20"><span className="flex items-center gap-3"><Icon size={20} className="text-primary"/>{label}</span><ChevronRight size={16} className="text-gray-400"/></Link>)}</div></aside><section className="lg:col-span-2"><div className="glass-card p-6"><div className="mb-6 flex items-center justify-between"><h2 className="flex items-center gap-2 text-xl font-semibold"><ShoppingBag className="text-primary"/> Recent Orders</h2><Link href={getStoreRouteHref(store, "/account/orders")} className="text-sm text-primary">View all</Link></div>{orders.length ? <div className="space-y-3">{orders.map((order) => <Link key={order.id} href={getStoreRouteHref(store, `/account/orders/${order.id}`)} className="flex items-center justify-between rounded-xl bg-black/5 p-4 dark:bg-white/5"><div><p className="font-semibold">#{order.id.slice(-8).toUpperCase()}</p><p className="text-sm text-gray-500">{new Date(order.createdAt).toLocaleDateString()}</p></div><span className="rounded-full bg-primary/10 px-2 py-1 text-xs text-primary">{order.status.replaceAll("_", " ")}</span><b>KES {order.total.toLocaleString()}</b></Link>)}</div> : <p className="py-8 text-center text-gray-500">No orders yet. <Link href={getStoreRouteHref(store, "/products")} className="text-primary">Start shopping.</Link></p>}</div></section></div></div>
}
