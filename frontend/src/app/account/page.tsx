...
import { Bell, ChevronRight, Heart, LogIn, LogOut, MapPin, Package, Settings, ShoppingBag, User, UserPlus } from "lucide-react"
...
export default function AccountPage() {
	...
	if (!user) return <div className="mx-auto max-w-lg py-12"><div className="glass-card p-8 text-center sm:p-10"><div className="mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-full bg-primary/10"><User size={32} className="text-primary" /></div><h1 className="text-3xl font-bold">Welcome to your account</h1><p className="mt-3 text-gray-500">Sign in to view orders and manage your account, or create a new account to get started.</p><div className="mt-8 grid gap-3 sm:grid-cols-2"><Link href="/auth/signin" className="btn-primary inline-flex items-center justify-center gap-2"><LogIn size={18} /> Sign In</Link><Link href="/auth/signup" className="inline-flex items-center justify-center gap-2 rounded-lg border border-primary px-4 py-2 font-semibold text-primary transition hover:bg-primary/10"><UserPlus size={18} /> Sign Up</Link></div></div></div>
