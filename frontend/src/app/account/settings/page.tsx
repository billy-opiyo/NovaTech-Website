...
export default function AccountSettingsPage() {
	...
	return <div className="mx-auto max-w-3xl space-y-8 py-6 sm:py-10">
		...
		<form onSubmit={save} className="space-y-6">
			...
			<div className="flex flex-wrap items-center gap-4"><button disabled={saving} className="btn-primary inline-flex items-center gap-2 disabled:opacity-50"><Save size={17} />{saving ? "Saving…" : "Save preferences"}</button><Link href="/account" className="text-sm text-gray-500 hover:text-primary">Back to account</Link></div>
		</form>
	</div>
}
