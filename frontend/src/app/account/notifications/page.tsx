...
export default function NotificationsPage() {
	...
	async function markRead(id?: string) {
		try { const response = await fetch("/api/account/notifications", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(id ? { id } : {}) }); if (!response.ok) throw new Error("Unable to mark notification as read"); setNotifications((items) => id ? items.map((item) => item.id === id ? { ...item, read: true } : item) : items.map((item) => ({ ...item, read: true }))); addToast("Notification marked as read", "success") } catch (error) { addToast(error instanceof Error ? error.message : "Unable to mark notification as read", "error") }
	}
