...
function daysUntil(value?: string | null) { if (!value) return null; return Math.ceil((new Date(value).getTime() - Date.now()) / (24 * 60 * 60 * 1000)) }
...
export default function ManageBillingPage() {
	...
	const pilotDaysLeft = daysUntil(data.subscription?.trialEndsAt)
	const graceActive = data.subscription?.status === "GRACE_PERIOD" || data.tenant.status === "GRACE_PERIOD"
	const storeSuspended = data.subscription?.status === "SUSPENDED" || data.tenant.suspensionReason === "BILLING"
	const renewalControls = data.subscription ? <div className="glass-card p-5"><h2 className="text-xl font-semibold">{firstPayment ? "Activate your paid plan" : "Renewal with M-Pesa"}</h2><p className="mt-2 text-sm text-gray-500">{trialActive ? `Your free Founding Merchant pilot is active until ${date(data.subscription.trialEndsAt)}${pilotDaysLeft != null && pilotDaysLeft >= 0 ? ` (${pilotDaysLeft} day${pilotDaysLeft === 1 ? "" : "s"} left)` : ""}. Nothing is charged automatically — choose and pay for a plan when the pilot ends.` : firstPayment ? "Your free pilot has ended. Pay the setup fee and first monthly subscription together by M-Pesa to activate your plan." : "M-Pesa renewals create an invoice and activate the next billing period only after callback confirmation."}</p>{data.paymentMethod?.shortcode ? <p className="mt-2 text-sm text-gray-500">Payment requests are sent from Nurava Tech PayBill <span className="font-semibold">{data.paymentMethod.shortcode}</span>{data.paymentMethod.env === "sandbox" ? " (sandbox)" : ""}. Enter your phone below and confirm with your M-Pesa PIN when prompted.</p> : <p className="mt-2 text-sm text-amber-700">Live M-Pesa collection is not configured yet; payment requests cannot be sent.</p>}<div className="mt-3 flex flex-wrap gap-2"><input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="07XXXXXXXX" className="rounded-lg border p-2 dark:bg-dark-surface"/><button disabled={!!busy || !phone || trialActive} onClick={() => action("renew_mpesa", { phone })} className="inline-flex items-center gap-2 rounded-lg border border-primary px-4 py-2 font-semibold text-primary">{busy === "renew_mpesa" && <Loader2 size={16} className="animate-spin" aria-hidden="true" />}{trialActive ? "Free pilot active" : firstPayment ? "Pay with M-Pesa" : "Renew with M-Pesa"}</button></div></div> : null
	return <div className="space-y-6">
		...
		{graceActive && <div className="rounded-lg border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900">Your free Founding Merchant pilot ended. A 14-day grace period is active{data.subscription?.gracePeriodEndsAt ? ` until ${date(data.subscription.gracePeriodEndsAt)}` : ""}. Your storefront stays available during the grace period — choose and pay for a plan below to keep it running. Nothing is charged automatically.</div>}
		{storeSuspended && <div className="rounded-lg border border-red-300 bg-red-50 p-4 text-sm text-red-900">Your public storefront is paused because no plan is active. Your data and workspace are preserved — choose and pay for a plan below to restore full operation.</div>}
		{renewalControls}
