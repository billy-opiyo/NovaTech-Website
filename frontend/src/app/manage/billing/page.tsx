...
type BillingData = { tenant: any; subscription: any; activeAddons: any[]; pendingAddons?: any[]; billingRecord: any; invoices: any[]; payments: any[]; usage: any[]; plans: Plan[]; addons: Addon[] }
...
export default function ManageBillingPage() {
	...
	const renewalControls = data.subscription ? <div className="glass-card p-5"><h2 className="text-xl font-semibold">{firstPayment ? "Activate your paid plan" : "Renewal with M-Pesa"}</h2><p className="mt-2 text-sm text-gray-500">{trialActive ? `Your 30-day trial is active until ${date(data.subscription.trialEndsAt)}. Payment becomes available after the trial ends.` : firstPayment ? "Pay the setup fee and first monthly subscription together by M-Pesa." : "M-Pesa renewals create an invoice and activate the next billing period only after callback confirmation."}</p><div className="mt-3 flex flex-wrap gap-2"><input value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="07XXXXXXXX" className="rounded-lg border p-2 dark:bg-dark-surface"/><button disabled={!!busy || !phone || trialActive} onClick={() => action("renew_mpesa", { phone })} className="rounded-lg border border-primary px-4 py-2 font-semibold text-primary">{trialActive ? "Trial active" : firstPayment ? "Pay with M-Pesa" : "Renew with M-Pesa"}</button></div></div> : null
