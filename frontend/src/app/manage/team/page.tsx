...
export default function TeamPage() {
	...
	async function invite(event: FormEvent) {
		...
		try { const response = await fetch("/api/manage/team/invitations", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email, role }) }); const data = await response.json().catch(() => ({})); if (!response.ok) setError(data.message || "Unable to create invitation"); else { setInviteLink(data.inviteLink); setMessage("Invitation created. Copy the link and send it to the invited person through an approved channel."); setEmail(""); await load() } } catch { setError("Unable to create invitation") } finally { setBusy(false) }
	}
