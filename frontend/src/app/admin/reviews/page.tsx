...
export default function AdminReviewsPage() {
	...
	const [busyReviewAction, setBusyReviewAction] = useState<string | null>(null)
	...
	async function moderate(id: string, moderationStatus: string) {
		setBusy(true); setBusyReviewId(id); setBusyReviewAction(moderationStatus)
		try {
		...
		} finally { setBusy(false); setBusyReviewId(null); setBusyReviewAction(null) }
	}
	...
	return (
		<div className="space-y-6">
			...
			<div className="glass-card overflow-x-auto"><table className="w-full min-w-[1050px]"><thead><tr className="border-b text-left text-sm text-gray-500"><th className="p-4">Product</th><th className="p-4">Customer</th><th className="p-4">Rating</th><th className="p-4">Review</th><th className="p-4">Status</th><th className="p-4">Actions</th></tr></thead><tbody>{data.reviews.map((review) => { const target = review.product?.name || "Store homepage"; return <tr key={review.id} className="border-b align-top last:border-0"><td className="p-4">{target}{!review.product && <span className="ml-2 text-xs text-gray-500">(homepage)</span>}</td><td className="p-4"><p>{review.user.name || "—"}</p><p className="text-xs text-gray-500">{review.user.email}</p></td><td className="p-4">{review.rating}/5</td><td className="max-w-xs p-4 text-sm"><p className="font-medium">{review.title || "—"}</p><p>{review.comment || "—"}</p></td><td className="p-4 text-xs">{review.moderationStatus}</td><td className="p-4"><div className="flex flex-wrap gap-2"><button type="button" disabled={busy} onClick={() => setEditReview({ id: review.id, rating: review.rating, title: review.title || "", comment: review.comment || "" })} className="rounded border px-2 py-1 text-xs">Edit</button><button type="button" disabled={busy} onClick={() => void moderate(review.id, "APPROVED")} className="inline-flex items-center gap-1 rounded border px-2 py-1 text-xs text-green-600">{busyReviewId === review.id && busyReviewAction === "APPROVED" && <Loader2 size={13} className="animate-spin" aria-hidden="true" />}Approve</button><button type="button" disabled={busy} onClick={() => void moderate(review.id, "FLAGGED")} className="inline-flex items-center gap-1 rounded border px-2 py-1 text-xs text-orange-600">{busyReviewId === review.id && busyReviewAction === "FLAGGED" && <Loader2 size={13} className="animate-spin" aria-hidden="true" />}Flag</button><button type="button" disabled={busy} onClick={() => setReviewToReject({ id: review.id, target })} className="destructive-action rounded border px-2 py-1 text-xs">Reject</button><button type="button" disabled={busy} onClick={() => setReviewToDelete({ id: review.id, target })} className="destructive-action rounded border px-2 py-1 text-xs">Delete</button></div></td></tr> })}</tbody></table>{!data.reviews.length && <p className="p-10 text-center text-gray-500">No reviews in this queue.</p>}</div>
		</div>
	)
}
