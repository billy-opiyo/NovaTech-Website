...
export default function AdminInventoryPage() {
	...
	async function update(item: InventoryItem) { const value = Number(window.prompt(`New stock for ${item.name}`, String(item.currentStock))); if (!Number.isInteger(value) || value < 0) return; const response = await fetch("/api/inventory", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(item.variantId ? { variantId: item.variantId, newStock: value } : { productId: item.id, newStock: value }) }); if (!response.ok) { setError("Unable to update stock"); return }; await load() }
