...
type InventoryItem = { id: string; name: string; sku: string; category: string; currentStock: number; threshold: number; price: number; image: string; variant?: string }
...
export default function AdminInventoryPage() {
	...
	async function update(item: InventoryItem) { const value = Number(window.prompt(`New stock for ${item.name}`, String(item.currentStock))); if (!Number.isInteger(value) || value < 0) return; const response = await fetch("/api/inventory", { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ productId: item.id, newStock: value }) }); if (!response.ok) { setError("Unable to update stock"); return }; await load() }
