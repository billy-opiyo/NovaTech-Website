...
export default function PlatformOperationsPage() {
	...
	const [pendingOperation, setPendingOperation] = useState<{ tenantId: string; action: "suspend_store" | "reject_verification"; name: string } | null>(null)
	...
	function requestStoreStatus(tenant: TenantRow, action: "suspend_store" | "reject_verification") {
		setPendingOperation({ tenantId: tenant.id, action, name: tenant.store?.name || tenant.legalName || tenant.id })
	}
	...
	return (
		<div className="space-y-6">
			<ConfirmDialog
				...
				title={pendingOperation?.action === "suspend_store" ? "Suspend this store?" : "Reject merchant verification?"}
				description={pendingOperation?.action === "suspend_store" ? `${pendingOperation?.name} will stop serving customers until it is reactivated.` : `${pendingOperation?.name} will remain unavailable until the merchant submits acceptable verification evidence.`}
				confirmLabel={pendingOperation?.action === "suspend_store" ? "Suspend store" : "Reject verification"}
				busy={Boolean(pendingOperation && busyTenant === pendingOperation.tenantId)}
				onCancel={() => { if (!busyTenant) setPendingOperation(null) }}
				onConfirm={() => {
					if (!pendingOperation) return
					const operation = pendingOperation
					setPendingOperation(null)
					void changeStoreStatus(operation.tenantId, operation.action)
				}}
			/>
			<div>
