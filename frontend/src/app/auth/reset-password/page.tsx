...
function ResetPasswordForm() {
	...
	async function submit(event: React.FormEvent) { event.preventDefault(); setError(""); try { const response = await fetch("/api/auth/reset-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ token, password }) }); const result = await response.json(); if (!response.ok) { setError(result.message); addToast(result.message || "Unable to update password", "error") } else { setDone(true); addToast("Password updated successfully", "success"); setTimeout(() => router.push("/auth/signin"), 1200) } } catch { setError("Unable to update password"); addToast("Unable to update password", "error") } }
