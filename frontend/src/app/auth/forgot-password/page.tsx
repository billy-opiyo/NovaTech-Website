...
export default function ForgotPasswordPage() {
	...
	async function submit(event: React.FormEvent) {
		...
		try { const response = await fetch("/api/auth/forgot-password", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email }) }); if (!response.ok) { setError("Unable to send reset instructions"); addToast("Unable to send reset instructions", "error") } else { setSubmitted(true); addToast("Reset instructions sent", "success") } } catch { setError("Unable to send reset instructions"); addToast("Unable to send reset instructions", "error") }
	}
