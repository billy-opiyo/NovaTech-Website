...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const key = generateTenantFileKey(context.tenantId, context.storeId, "general", `category-${slot}.webp`)
		...
		console.error("Store category image upload error:", error)
		return NextResponse.json({ message: "Unable to upload the category image." }, { status: 500 })
	}
}
