...
export async function GET(request: NextRequest) {
	try {
		...
		const products = await prisma.product.findMany({ where: { tenantId: context.tenantId }, orderBy: { createdAt: "asc" }, include: { category: { select: { name: true, slug: true } }, variants: { select: { name: true, value: true, priceModifier: true, stock: true, sku: true } } } })
	...
	} catch (error: any) { return NextResponse.json({ message: error.message || "Catalog export unavailable" }, { status: error.status || 503 }) }
}
