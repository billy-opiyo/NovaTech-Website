...
export async function POST(request: NextRequest) {
	...
	try {
		...
		const key = `platform/assets/logo-${Date.now()}-${Math.random().toString(36).slice(2, 9)}.webp`
		...
		return NextResponse.json({ message: "Unable to upload the platform logo." }, { status: 500 })
	}
}
