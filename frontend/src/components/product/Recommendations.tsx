...
export default function Recommendations({ productId }: { productId: string }) {
	...
	return (
		<section className="border-t border-gray-200 pt-8 dark:border-gray-700">
			<div className="mb-5 flex items-end justify-between gap-4"><div><h2 className="text-2xl font-bold">You may also like</h2><p className="mt-1 text-sm text-gray-500">Similar products selected for this item</p></div><Link href="/products" className="text-sm font-medium text-primary hover:underline">Browse all</Link></div>
			<div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{products.map((product) => <article key={product.id} className="glass-card overflow-hidden"><Link href={`/products/${product.slug}`} className="block"><div className="relative aspect-square bg-gray-100 dark:bg-gray-800"><Image src={getProductImage(product.images[0], product.name)} alt={product.name} fill className="object-cover" /></div><div className="p-4"><p className="line-clamp-2 min-h-10 font-semibold">{product.name}</p><div className="mt-2 flex items-center gap-1 text-sm text-gray-500"><Star size={14} className="fill-yellow-500 text-yellow-500" />{product.rating > 0 ? product.rating.toFixed(1) : "New"}<span>({product.reviewCount})</span></div><div className="mt-3 flex items-baseline gap-2"><span className="font-bold text-primary">KES {(product.discountedPrice ?? product.price).toLocaleString()}</span>{product.discountedPrice && <span className="text-xs text-gray-400 line-through">KES {product.price.toLocaleString()}</span>}</div></div></Link></article>)}</div>
		</section>
	)
}
