		...
		<section className="relative rounded-3xl overflow-hidden glass-card navy-glass p-6 sm:p-8 md:p-16 text-center">
			...
			>
				<h1 className="text-3xl sm:text-4xl md:text-6xl font-extrabold mb-4 leading-tight">
					...
					<span className="block text-primary">{store.homepage.heroHighlight}</span>
				</h1>
				<p className="text-base sm:text-lg md:text-xl text-gray-600 dark:text-gray-300 mb-8 max-w-2xl mx-auto">
					{store.homepage.heroDescription}
				</p>
				<div className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center items-center">
					<Link
						href={store.homepage.heroPrimaryHref}
						className="btn-primary flex items-center justify-center gap-2 w-full sm:w-auto"
					>
						{store.homepage.heroPrimaryLabel} <ArrowRight size={18} />
					</Link>
					<Link
						href={store.homepage.heroSecondaryHref}
					...
					>
						{store.homepage.heroSecondaryLabel}
					</Link>
				</div>
			</motion.div>
		</section>
