...
function PreviewButton({ children, secondary = false }: { children: React.ReactNode; secondary?: boolean }) {
	return <span className={`inline-flex items-center justify-center rounded px-2.5 py-1.5 text-[10px] font-bold ${secondary ? "border border-sky-400/40 text-sky-300" : "bg-primary text-white"}`}>{children}</span>
}
...
function PreviewShell({ kind, children }: { kind: Exclude<GuidePreviewKind, "auth" | "plans" | "onboarding">; children: React.ReactNode }) {
	...
	return <div className="overflow-hidden rounded-xl border border-sky-400/25 bg-[#0f172a] text-left shadow-inner" aria-hidden="true">
		<div className="flex items-center gap-2 border-b border-slate-700 bg-[#071a36] px-3 py-2"><span className="flex gap-1"><i className="h-1.5 w-1.5 rounded-full bg-red-300" /><i className="h-1.5 w-1.5 rounded-full bg-amber-300" /><i className="h-1.5 w-1.5 rounded-full bg-emerald-300" /></span><span className="min-w-0 truncate rounded bg-[#1e293b] px-2 py-1 text-[9px] text-slate-400">{previewPageNames[kind]}</span></div>
		<div className="grid min-h-[205px] grid-cols-[5rem_minmax(0,1fr)] sm:grid-cols-[7rem_minmax(0,1fr)]">
			<aside className="border-r border-slate-700 bg-[#1e293b] p-2 text-[9px] text-slate-400"><p className="mb-3 px-1 font-bold text-slate-100">Admin</p>{["Dashboard", "Products", "Store design", "Verification", "Subscription", "Launch readiness"].map((item) => <div key={item} className={`mb-1 rounded px-1.5 py-1 ${item === active ? "bg-primary text-white" : ""}`}>{item}</div>)}</aside>
			<div className="min-w-0 bg-[#0f172a] p-3 sm:p-4">{children}</div>
		</div>
	</div>
}

function MerchantPagePreview({ kind, plans }: { kind: GuidePreviewKind; plans: PublicPlan[] }) {
	if (kind === "auth") return <div className="overflow-hidden rounded-xl border border-sky-400/25 bg-[#0f172a] p-4 shadow-inner" aria-hidden="true"><div className="mx-auto max-w-[18rem] rounded-xl border border-slate-700 bg-[#1e293b] p-4 shadow-sm"><div className="text-center"><p className="text-base font-bold text-slate-100">Welcome Back</p><p className="mt-1 text-[10px] text-slate-400">Sign in to your account to continue</p></div><div className="mt-3 flex items-center justify-center gap-2 rounded-lg border border-slate-600 px-2 py-2 text-[10px] font-semibold text-slate-200"><span className="font-bold text-blue-400">G</span> Continue with Google</div><div className="my-3 flex items-center gap-2 text-[8px] text-slate-500"><span className="h-px flex-1 bg-slate-700" />or continue with email<span className="h-px flex-1 bg-slate-700" /></div><PreviewInput label="Email" value="you@example.com" /><PreviewInput label="Password" value="••••••••" className="mt-2" /><div className="mt-3"><PreviewButton>Sign In</PreviewButton></div></div></div>
	if (kind === "plans") return <div className="overflow-hidden rounded-xl border border-sky-400/25 bg-[#0f172a] p-3 shadow-inner" aria-hidden="true"><div className="text-center"><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-primary">Plans for store partners</p><p className="mt-1 text-base font-extrabold text-slate-100">Simple billing, clear capacity</p></div><div className="mt-3 grid gap-2 sm:grid-cols-3">{plans.length ? plans.slice(0, 3).map((plan, index) => <div key={plan.key} className={`rounded-lg border bg-[#1e293b] p-2 ${plan.key === "BUSINESS" || (!plans.some((item) => item.key === "BUSINESS") && index === 1) ? "border-primary ring-1 ring-primary/20" : "border-slate-700"}`}><p className="text-[9px] font-bold uppercase text-primary">{plan.name}</p><p className="mt-1 text-sm font-extrabold text-slate-100">{plan.price == null ? "Contact us" : `${plan.currency} ${plan.price.toLocaleString()}`}{plan.price != null && plan.billingInterval ? <span className="text-[8px] font-medium text-slate-400"> / {plan.billingInterval.toLowerCase()}</span> : null}</p><p className="mt-1 text-[8px] text-slate-400">One-time setup: {plan.currency} {plan.setupFeeAmount.toLocaleString()}</p><div className="mt-2"><PreviewButton secondary>Choose {plan.name}</PreviewButton></div></div>) : <div className="rounded-lg border border-amber-400/40 bg-amber-950/40 p-3 text-[9px] text-amber-200 sm:col-span-3">No public plans are currently configured.</div>}</div></div>
	if (kind === "onboarding") return <div className="overflow-hidden rounded-xl border border-sky-400/25 bg-[#0f172a] p-4 shadow-inner" aria-hidden="true"><div className="mx-auto max-w-[26rem] rounded-xl border border-slate-700 bg-[#1e293b] p-4 shadow-sm"><p className="text-[9px] font-bold uppercase tracking-[0.16em] text-primary">Nurava Tech SaaS</p><p className="mt-1 text-base font-extrabold text-slate-100">Create your store</p><p className="mt-1 text-[9px] text-slate-400">Set up the store identity first. Products, design, payments, and publishing follow in the workspace.</p><div className="mt-3 grid gap-2 sm:grid-cols-2"><PreviewInput label="Store name" value="Acme Electronics" /><PreviewInput label="Platform slug (optional)" value="acme-electronics" /><PreviewInput label="Choose a plan" value="Business · KES 2,500 / month" className="sm:col-span-2" /></div><p className="mt-3 flex items-start gap-1 text-[8px] text-slate-400"><Check size={10} className="mt-0.5 shrink-0 text-emerald-500" /> I confirm that I reviewed the merchant terms and privacy notice.</p><div className="mt-3"><PreviewButton>Create store</PreviewButton></div></div></div>
	if (kind === "products") return <PreviewShell kind={kind}><div className="flex items-center justify-between gap-2"><div><p className="text-base font-extrabold text-slate-100">Products</p><p className="text-[9px] text-slate-400">Manage your electronics catalog, pricing, specifications, and galleries.</p></div><PreviewButton>+ Add product</PreviewButton></div><div className="mt-3 flex items-center gap-1 rounded border border-slate-700 bg-[#1e293b] px-2 py-1.5 text-[9px] text-slate-400"><Search size={11} /> Search by name, brand, SKU, or category</div><div className="mt-3 rounded border border-slate-700 bg-[#1e293b] p-2"><p className="text-[9px] font-bold text-slate-200">Add product</p><div className="mt-2 grid grid-cols-2 gap-2"><PreviewInput label="Name *" value="Product name" /><PreviewInput label="Brand *" value="Brand" /><PreviewInput label="Price (KES) *" value="0.00" /><PreviewInput label="Stock *" value="0" /></div><div className="mt-2 flex items-center gap-2"><FileText size={11} className="text-slate-500" /><span className="text-[9px] text-slate-400">Description · Gallery image URLs · Specifications</span></div></div></PreviewShell>
	if (kind === "design") return <PreviewShell kind={kind}><div className="flex items-center justify-between gap-2"><div><p className="text-base font-extrabold text-slate-100">Store design</p><p className="text-[9px] text-slate-400">Customize your public storefront and save a draft.</p></div><Settings2 size={16} className="text-primary" /></div><div className="mt-3 grid gap-2 sm:grid-cols-2"><div className="rounded border border-slate-700 bg-[#1e293b] p-2"><p className="text-[9px] font-bold text-slate-200">Theme preset</p><div className="mt-2 grid grid-cols-2 gap-1"><span className="rounded border border-primary bg-primary/10 p-1 text-[8px] text-sky-300">Modern</span><span className="rounded border border-slate-600 p-1 text-[8px] text-slate-400">Classic</span></div><PreviewInput label="Store name" value="Acme Electronics" className="mt-2" /><PreviewInput label="Hero title" value="Shop our collection" className="mt-2" /></div><div className="rounded border border-primary/30 bg-[#1e293b] p-2"><p className="text-[9px] font-bold text-sky-300">Featured storefront</p><p className="mt-2 text-sm font-extrabold text-slate-100">Shop our collection</p><p className="mt-1 text-[9px] text-slate-400">Reliable products, delivered to you.</p><span className="mt-3 inline-block rounded bg-primary px-2 py-1 text-[8px] font-bold text-white">Shop the collection</span></div></div><div className="mt-2 flex gap-2"><PreviewButton>Save draft</PreviewButton><PreviewButton secondary>Publish draft</PreviewButton></div></PreviewShell>
	if (kind === "verification") return <PreviewShell kind={kind}><div><p className="text-base font-extrabold text-slate-100">Merchant verification</p><p className="text-[9px] text-slate-400">Complete these checks before your store can be published or sell.</p></div><div className="mt-3 flex items-center justify-between rounded border border-amber-400/40 bg-amber-950/40 p-2"><div><p className="text-[8px] text-slate-400">Current status</p><p className="text-xs font-bold text-slate-100">Action required</p></div><span className="rounded-full bg-amber-400/20 px-2 py-1 text-[8px] font-bold text-amber-200">Not submitted</span></div><div className="mt-2 rounded border border-slate-700 bg-[#1e293b] p-2"><p className="text-[9px] font-bold text-slate-200">Merchant details</p><div className="mt-2 grid grid-cols-2 gap-2"><PreviewInput label="Merchant type" value="Individual" /><PreviewInput label="Tax status" value="Registered / has KRA PIN" /><PreviewInput label="Legal name" value="Your legal name" /><PreviewInput label="Merchant phone" value="0712 345 678" /></div><div className="mt-2 flex flex-wrap gap-1"><PreviewButton>Save details and continue</PreviewButton><span className="inline-flex items-center gap-1 rounded border border-slate-600 px-2 py-1 text-[8px] text-slate-400"><Upload size={10} /> Verification documents</span></div></div></PreviewShell>
	if (kind === "billing") return <PreviewShell kind={kind}><div><p className="text-base font-extrabold text-slate-100">Subscription and billing</p><p className="text-[9px] text-slate-400">Manage the merchant workspace subscription separately from shopper checkout.</p></div><div className="mt-3 rounded border border-primary/30 bg-primary/10 p-2"><p className="text-[10px] font-bold text-slate-100">Activate your paid plan</p><p className="mt-1 text-[8px] text-slate-400">Pay the setup fee and first monthly subscription together by M-Pesa.</p><div className="mt-2 flex gap-1"><PreviewInput label="Merchant phone" value="07XXXXXXXX" className="flex-1" /><PreviewButton>Pay with M-Pesa</PreviewButton></div></div><div className="mt-2 grid grid-cols-3 gap-1">{[["Current plan", "Business"], ["Setup fee", "PENDING"], ["Shopper fees", "Not applied"]].map(([label, value]) => <div key={label} className="rounded border border-slate-700 bg-[#1e293b] p-2"><p className="text-[8px] text-slate-400">{label}</p><p className="mt-1 text-[10px] font-bold text-slate-100">{value}</p></div>)}</div><div className="mt-2 flex items-center gap-1 text-[8px] text-slate-400"><WalletCards size={11} /> Invoices and payment history appear below.</div></PreviewShell>
	if (kind === "readiness") return <PreviewShell kind={kind}><div><p className="text-base font-extrabold text-slate-100">Launch readiness</p><p className="text-[9px] text-slate-400">Review the server-backed checks required before publishing.</p></div><div className="mt-3 rounded border border-amber-400/40 bg-amber-950/40 p-2"><p className="text-xs font-bold text-amber-200">Action required before publication</p><p className="mt-1 text-[8px] text-amber-300">Resolve every pending or failed check before publishing.</p></div><div className="mt-2 divide-y divide-slate-700 rounded border border-slate-700 bg-[#1e293b] px-2">{["Store content", "Legal acceptance", "Merchant verification", "Access requirements"].map((item, index) => <div key={item} className="flex items-center justify-between py-2 text-[9px] text-slate-300"><span>{item}</span><span className={`rounded-full px-1.5 py-0.5 text-[8px] font-bold ${index === 0 ? "bg-emerald-400/20 text-emerald-300" : "bg-amber-400/20 text-amber-200"}`}>{index === 0 ? "PASS" : "PENDING"}</span></div>)}</div><div className="mt-2"><PreviewButton secondary>Refresh checks</PreviewButton></div></PreviewShell>
	return <PreviewShell kind="dashboard"><div className="flex items-center justify-between gap-2"><div><p className="text-base font-extrabold text-slate-100">Dashboard</p><p className="text-[9px] text-slate-400">Welcome back, Admin! Here&apos;s what&apos;s happening.</p></div><div className="flex gap-1"><PreviewButton secondary>View Store</PreviewButton><PreviewButton>View Report</PreviewButton></div></div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">{[["Revenue (7 days)", "KES 0"], ["Orders (7 days)", "0"], ["Average order value", "KES 0"], ["Paid order rate", "0.00%"]].map(([label, value]) => <div key={label} className="rounded border border-primary/30 bg-[#1e293b] p-2"><p className="text-sm font-extrabold text-slate-100">{value}</p><p className="mt-1 text-[8px] text-slate-400">{label}</p></div>)}</div><div className="mt-2 rounded border border-slate-700 bg-[#1e293b] p-3"><p className="text-[10px] font-bold text-slate-200">Sales Overview</p><p className="mt-5 text-center text-[9px] text-slate-400">Live revenue and order charts are available in Analytics.</p></div></PreviewShell>
}
		...
		<section aria-labelledby="merchant-onboarding-guide-title" className="rounded-3xl border border-primary/20 bg-gradient-to-br from-primary/[0.1] via-transparent to-accent/[0.08] p-5 shadow-xl sm:p-8" onMouseEnter={() => setPaused(true)} onMouseLeave={() => setPaused(false)} onFocusCapture={() => setPaused(true)} onBlurCapture={(event) => { if (!event.currentTarget.contains(event.relatedTarget)) setPaused(false) }}>
			...
			<div className="mt-8 grid gap-6 lg:grid-cols-[minmax(12rem,0.7fr)_minmax(0,1.7fr)]">
				...
				<article aria-live="polite" className="glass-card min-w-0 overflow-hidden border border-white/20 p-5 sm:p-7">
					...
					<div className="mt-6"><p className="mb-2 text-[10px] font-bold uppercase tracking-[0.16em] text-gray-500">Actual page preview</p><MerchantPagePreview kind={step.preview} plans={plans} /></div>
