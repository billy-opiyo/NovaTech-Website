...
export default function RecentOrders() {
	...
	return (
		<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="glass-card p-6 lg:col-span-2">
			<div className="mb-6 flex items-center justify-between"><div><h2 className="text-xl font-bold">Recent Orders</h2><p className="mt-1 text-sm text-gray-500">Latest live transactions</p></div><Link href={`${basePath}/orders`} className="text-sm text-primary hover:underline">View All</Link></div>
			...
			{state === "ready" && orders.length > 0 && <div className="space-y-4">{orders.map((order) => { const product = order.items[0]?.product?.name || "Order"; return <Link key={order.id} href={`${basePath}/orders`} className="flex items-center gap-4 rounded-xl p-2 transition hover:bg-black/5 dark:hover:bg-white/5"><div className="min-w-0 flex-1"><p className="truncate text-sm font-medium">{product}</p><p className="text-xs text-gray-500">{order.shippingAddress?.fullName || "Guest"} · {new Date(order.createdAt).toLocaleString()}</p></div><div className="flex-shrink-0 text-right"><p className="text-sm font-semibold">KES {order.total.toLocaleString()}</p><span className={clsx("inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-medium", getStatusBadge(order.status))}>{getStatusIcon(order.status)}{order.status.replaceAll("_", " ")}</span></div></Link> })}</div>}
		</motion.div>
	)
}
