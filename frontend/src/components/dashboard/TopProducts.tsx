...
export default function TopProducts() {
	...
	return (
		<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="glass-card p-6">
			...
			{state === "ready" && products.length > 0 && <div className="space-y-4">{products.map((product) => <div key={product.id} className="flex items-center gap-4"><div className="relative h-16 w-16 flex-shrink-0 overflow-hidden rounded-lg bg-gray-200 dark:bg-gray-700"><Image src={getProductImage(product.image, product.name)} alt={product.name} fill className="object-cover" /></div><div className="min-w-0 flex-1"><Link href={`/products/${product.id}`} className="block truncate text-sm font-medium hover:text-primary">{product.name}</Link><p className="mt-1 text-xs text-gray-500">{product.sales} sold</p><p className="mt-1 text-sm font-semibold text-green-600">KES {product.revenue.toLocaleString()}</p></div></div>)}</div>}
			<Link href="/admin/products" className="mt-4 block w-full rounded-lg border border-gray-300 px-4 py-2 text-center text-sm transition hover:bg-gray-100 dark:border-gray-600 dark:hover:bg-gray-700">View All Products</Link>
		</motion.div>
	)
}
