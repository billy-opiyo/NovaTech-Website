...
3. Update `client.config.ts` only when changing the safe default or seeded Nurava Tech fallback, then set `themePreset` to one of the IDs exported in `theme-presets.ts` when a code-level preset change is intended.
