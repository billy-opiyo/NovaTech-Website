...
async function main() {
	...
	const user = await prisma.user.upsert({ where: { email }, update: { passwordHash, role: Role.SUPERADMIN }, create: { email, passwordHash, role: Role.SUPERADMIN, name: "Super Administrator" } })
