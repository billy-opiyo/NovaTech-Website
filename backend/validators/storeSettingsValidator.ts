...
export const storeSettingsPatchSchema = z.object({
	...
	contact: z.object({ phoneDisplay: z.string().trim().max(40).optional(), email: z.string().email().optional(), whatsappNumber: z.string().regex(/^\d{10,15}$/).optional(), addressLine: z.string().trim().max(160).optional(), cityCountry: z.string().trim().max(120).optional(), businessHours: z.string().trim().max(120).optional(), responseTime: z.string().trim().max(120).optional() }).strict().optional(),
